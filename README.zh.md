...
每行除 `skew_factor` 外还带 `input_skew_factor` 与 `wall_share`。`data_skew` 现在有四道闸门:**任务时长紧致**(`p99/p50 < 1.5`)直接 ok;输入均匀(`input_skew_factor < 1.2` 且 `p99/p50 < 20`);stage `wall_share < 1%`;命中 idle stage 条件(busy < 0.2)。`slow-stages` 行带 `gc_ratio`、`busy_ratio`、三档 `*_mb_per_task`,以及 sparkMeasure 语义的 AI 信号:`scheduler_delay_ratio`、`remote_shuffle_read_ratio`、`executor_cpu_ratio`、`speculative_tasks`、`peak_execution_memory_gb`。`app-summary` 同时给 `top_stages_by_duration[]`、`top_busy_stages[]`、`top_io_bound_stages[]`,并输出 `avg_active_tasks`、`executor_cpu_ratio`、`scheduler_delay_ratio`、`remote_shuffle_read_ratio` 等全局分流比例。`diagnose.summary` 加 `top_findings_by_impact` 与 `findings_wall_coverage`;新增 `scheduler_delay`、`remote_shuffle`、`speculative_tasks` finding,仍按 wall impact 排 ROI。每行重复嵌入的 SQL 文本上提到 envelope 顶层 `sql_executions: {<id>: <description>}` 一份共享(仅 slow-stages / data-skew);DataFrame 作业那种 callsite 占位文本会被整行过滤。
...
如果 Spark 应用启用了 Paimon diagnostics/profiler 插件,可以直接抓取该 tab 面向
AI 的 JSON:
...
该命令读取 Spark UI 的 `/paimon-diagnostics/json`、
`/paimon-diagnostics/threadDump/json?executorId=<id>` 与
`/paimon-diagnostics/profiler/json`。输出里的 `overview`、`thread_dump`、
`profiler` 是 Paimon tab 原生 JSON,agent 应优先读
`thread_dump.facts.state_counts`、`thread_dump.facts.top_stacks`、
`thread_dump.facts.flamegraph` 和 `profiler.facts.artifacts`,不要再解析 HTML。
端点缺失会进入 `warnings`,不会让整个命令失败。
...
`--dry-run`、`--guided`、`--log-dirs`、`--cluster`、`--cache-dir`、`--no-cache`、
...
Agent 客户端加载对应 skill 后会遵循「先确认集群,再 guided diagnose,最后下钻」的标准流程。
...
- `diagnose` 新增 `scheduler_delay` / `remote_shuffle` / `speculative_tasks` 规则。三者 evidence 都带 `stage_id` 和可用时的 `wall_share`;这是给 AI 的下一步路由信号,分别指向 executor 供给/locality/小 task、数据本地性/网络/shuffle 布局、慢节点/倾斜/GC 抖动。
...
- `app-summary` 额外输出 AI 分流字段:`avg_active_tasks`、`executor_cpu_ratio`、`scheduler_delay_ratio`、`remote_shuffle_read_ratio`、`speculative_tasks`、`peak_execution_memory_gb`。
- `slow-stages` row 额外输出 AI 归因字段:`executor_cpu_ratio`、`scheduler_delay_ms`、`scheduler_delay_ratio`、`remote_shuffle_read_ratio`、`shuffle_remote_blocks`、`shuffle_total_blocks`、records 计数、`peak_execution_memory_gb`、`speculative_tasks`。
