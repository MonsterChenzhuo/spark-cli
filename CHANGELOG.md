...
- **New `diagnose --guided` SOP preflight** confirms cluster selection before reading EventLogs. It auto-selects the only configured cluster, fails when multiple clusters exist without `--cluster` / `active_cluster`, returns a `config cluster add ... --activate` hint when no source is configured, and keeps stdout as the normal `diagnose` JSON envelope while writing preflight notes to stderr as JSON events.
- **`--guided` is now diagnose-only at the CLI surface.** `spark-cli diagnose --help` exposes it, while other commands reject `--guided` with `FLAG_INVALID` instead of silently accepting a no-op flag.
...
- **New `spark-cli paimon-diagnostics <appId>`** fetches the Paimon diagnostics tab's AI-readable JSON through the YARN tracking/proxy URL. It returns `overview`, `thread_dump`, and `profiler` payloads from `/paimon-diagnostics/*/json`, including thread-dump aggregate flamegraph facts and async-profiler artifact metadata without parsing HTML.
...
- **SHS first-fetch progress on stderr**: `bundleFor` emits `SHS_DOWNLOAD_START` before the HTTP body and `SHS_DOWNLOAD_READY` afterwards as JSON events. Set `SPARK_CLI_QUIET=1` to silence (intended for scripted callers / tests). Cache hits still need this fetch to detect V1/V2 layout — surfacing the wait removes the "is the CLI hung?" UX bug.
