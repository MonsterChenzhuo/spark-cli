...
func SlowStagesColumns() []string {
	return []string{
		...
		"busy_ratio", "shuffle_read_mb_per_task",
		"sql_execution_id",
	}
}
...
func SlowStages(app *model.Application, top int) []SlowStageRow {
	...
	for _, s := range app.Stages {
		...
		rows = append(rows, SlowStageRow{
			...
			ShuffleReadMBPerTask: shuffleReadMBPerTask(s),
...
func shuffleReadMBPerTask(s *model.Stage) float64 {
	if s.NumTasks <= 0 {
		return 0
	}
	return round3(float64(s.TotalShuffleReadBytes) / float64(s.NumTasks) / float64(mb))
}
