...
- **V2 attempt 后缀匹配**: Spark 实际写出的滚动日志目录是 `eventlog_v2_<appId>_<attempt>` (例如尾部 `_1`),不是裸 `eventlog_v2_<appId>`。`resolveV2` 用正则 `^eventlog_v2_<appId>(?:_(\d+))?$` 容忍可选 attempt,多 attempt 共存时取最大 (Spark History Server 行为)。**别**把它改回精确等值 —— 真实 EventLog 几乎都带 attempt,改回去会全线 APP_NOT_FOUND。
