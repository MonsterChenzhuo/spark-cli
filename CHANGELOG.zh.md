...
- **新增 `spark-cli paimon-diagnostics <appId>`**:通过 YARN tracking/proxy URL 拉取 Paimon diagnostics tab 的 AI 友好 JSON。输出包含 `/paimon-diagnostics/*/json` 的 `overview`、`thread_dump`、`profiler`,直接暴露 thread dump 聚合火焰图事实与 async-profiler artifact 元数据,不需要解析 HTML。
