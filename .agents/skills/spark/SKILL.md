...
`contract_version` 当前固定为 `1`;不认识版本时不要猜字段含义。`app_duration_ms` 是应用 wall 时长(`SparkListenerApplicationEnd - SparkListenerApplicationStart`),omitempty 在 `app.DurationMs == 0`(没 ApplicationEnd 事件)时缺失。所有 EventLog 场景一致输出 —— 看 `wall_share` 时直接换算绝对秒数,不必再额外跑 `app-summary`。
